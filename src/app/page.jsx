...
export default function page() {
  ...
  const list = filteredCourse.length > 0 ? filteredCourse.map(course => (
    <section key={course.id} className="flex flex-col item-center border-4 border-orange-500 p-4 rounded-xl">
      <Link href={`/course/${course.name}`}>
        <Image src={course.image} alt={course.name} width={250} height={250} />
        <h2 className="p-3"><span className="font-bold">course name:</span>{course.name}</h2>
        <p className="p-3"><span className="font-bold">course duration:</span>{course.duration}</p>
      </Link>
    </section>
  )) : (
    <h2>course does not exit</h2>
  )
  ...
  return (
    <div>
      <input type="text" name="text" id="text"
        ...
        className="w-1/2 m-auto py-4 my-5 outline-none border-sky-600 border-4 rounded-xl"
        value={item}
        onChange={onChange}
      />
      <div className="text-[40px] text-center grid md:grid-cols-3
grid-cols-1 gap-2">
        {list}
      </div>
      <div className='bg-[url(/html.png)] z-10 flex flex-col item-center border-4 mt-28 border-orange-500 p-4 rounded-xl h-[250px] w-[250px] '> 
      <a href="https://github.com/Sofiat0/6b3-wk1/" ></a> 
      <a href="https://github.com/Sofiat0/6B3-WK2/"></a>  
      <a href="https://github.com/Sofiat0/6b3wk3/"></a> 
       </div>
    </div>
